import Link from "next/link";
import {
  LayoutDashboard,
  BedDouble,
  Users,
  FileText,
  ShieldCheck,
  UserCog,
  DollarSign,
} from "lucide-react";
import { getServerSession } from "next-auth";
import { authOptions } from "@/app/api/auth/[...nextauth]/route";
import LogoutButton from "@/components/auth/LogoutButton";

export default async function AdminLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  const session = await getServerSession(authOptions);
  const userPermissions: string[] = session?.user?.permissions ?? [];
  const managed = session?.user?.managedProperties ?? [];
  const showPriceMenu = session?.user?.managedProperties?.some((p: any) => !p.isFree);

  // --- LOGIKA UNTUK JUDUL DINAMIS ---
  let sidebarTitle = "Panel Admin"; // default
  if (session?.user?.role !== "Super Administrator" && managed.length > 0) {
    sidebarTitle = managed.map((p) => p.name).join(", ");
  }

  // Data untuk mobile sidebar
  const sidebarData = {
    title: sidebarTitle,
    userName: session?.user?.name || "",
    userRole: session?.user?.role === "Super Administrator"
      ? "Super Administrator"
      : managed.map((p) => p.name).join(", "),
    permissions: userPermissions,
    showPriceMenu,
  };

  return (
    <div className="relative flex min-h-screen">
      {/* Mobile Sidebar - Client Component */}

      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-64 shrink-0 bg-gray-800 text-white flex-col overflow-y-auto">
        <div className="p-4">
          <h1 className="text-2xl font-bold mb-8">{sidebarTitle}</h1>

          <nav>
            <ul>
              {/* Dashboard */}
              <li className="mb-4">
                <Link
                  href="/admin"
                  className="flex items-center p-2 rounded hover:bg-gray-700"
                >
                  <LayoutDashboard className="h-5 w-5 mr-3" />
                  Dashboard
                </Link>
              </li>

              {/* Manajemen Kamar */}
              {userPermissions.includes("rooms:read") && (
                <li className="mb-4">
                  <Link
                    href="/admin/rooms"
                    className="flex items-center p-2 rounded hover:bg-gray-700"
                  >
                    <BedDouble className="h-5 w-5 mr-3" />
                    Manajemen Kamar
                  </Link>
                </li>
              )}

              {/* Manajemen Harga */}
              {userPermissions.includes("prices:read") && showPriceMenu && (
                <li className="mb-4">
                  <Link
                    href="/admin/prices"
                    className="flex items-center p-2 rounded hover:bg-gray-700"
                  >
                    <DollarSign className="h-5 w-5 mr-3" />
                    Manajemen Harga
                  </Link>
                </li>
              )}

              {/* Manajemen Denda */}
              {userPermissions.includes("charges:read") && (
                <li className="mb-4">
                  <Link
                    href="/admin/charges"
                    className="flex items-center p-2 rounded hover:bg-gray-700"
                  >
                    <DollarSign className="h-5 w-5 mr-3" />
                    Manajemen Denda
                  </Link>
                </li>
              )}

              {/* Tamu Aktif */}
              {userPermissions.includes("bookings:read") && (
                <li className="mb-4">
                  <Link
                    href="/admin/active-bookings"
                    className="flex items-center p-2 rounded hover:bg-gray-700"
                  >
                    <Users className="h-5 w-5 mr-3" />
                    Tamu Aktif
                  </Link>
                </li>
              )}

              {/* Laporan */}
              {userPermissions.includes("reports:read") && (
                <li className="mb-4">
                  <Link
                    href="/admin/laporan"
                    className="flex items-center p-2 rounded hover:bg-gray-700"
                  >
                    <FileText className="h-5 w-5 mr-3" />
                    Laporan
                  </Link>
                </li>
              )}

              {/* Manajemen Peran */}
              {userPermissions.includes("roles:read") && (
                <li className="mb-4">
                  <Link
                    href="/admin/roles"
                    className="flex items-center p-2 rounded hover:bg-gray-700"
                  >
                    <ShieldCheck className="h-5 w-5 mr-3" />
                    Manajemen Peran
                  </Link>
                </li>
              )}

              {/* Manajemen Pengguna */}
              {userPermissions.includes("users:read") && (
                <li className="mb-4">
                  <Link
                    href="/admin/users"
                    className="flex items-center p-2 rounded hover:bg-gray-700"
                  >
                    <UserCog className="h-5 w-5 mr-3" />
                    Manajemen Pengguna
                  </Link>
                </li>
              )}
            </ul>
          </nav>
        </div>

        {/* User Info & Logout */}
        <div className="mt-auto p-4 border-t border-gray-700">
          <div className="mb-4">
            <p className="font-semibold text-sm truncate">
              {session?.user?.name}
            </p>
            <p className="text-xs text-gray-400">
              {session?.user?.role === "Super Administrator"
                ? "Super Administrator"
                : managed.map((p) => p.name).join(", ")}
            </p>
          </div>
          <LogoutButton />
        </div>
      </aside>

      {/* Main Content */}
      <main className="flex-1 w-full min-w-0 p-4 md:p-8 bg-gray-100 dark:bg-gray-900">
        {children}
      </main>
    </div>
  );
}